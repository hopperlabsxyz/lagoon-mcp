...
describe('optimize_portfolio', () => {
  ...
  describe('Input Validation', () => {
    it('should validate vault addresses format', async () => {
      const invalidInput = {
        ...baseInput,
        vaultAddresses: ['invalid-address'],
      };

      const result = await executeOptimizePortfolio(invalidInput as never);

      expect(result.isError).toBe(true);
      expect(result.content[0]).toHaveProperty('type', 'text');
      expect(result.content[0]).toHaveProperty('text');
      expect((result.content[0] as { text: string }).text).toContain('Invalid Ethereum address');
    });

    it('should require at least 2 vault addresses', async () => {
      const invalidInput = {
        ...baseInput,
      ...
      expect(result.isError).toBe(true);
      expect((result.content[0] as { text: string }).text).toContain('At least 2 vault addresses');
    });

    it('should enforce maximum 20 vaults', async () => {
      const tooManyVaults = Array.from(
        { length: 21 },
        (_, i) => `0x${i.toString().padStart(40, '0')}`
      );
      const invalidInput = {
        ...baseInput,
        vaultAddresses: tooManyVaults,
        currentPositions: tooManyVaults.map((addr) => ({ vaultAddress: addr, valueUsd: 1000 })),
      };

      const result = await executeOptimizePortfolio(invalidInput as never);

      expect(result.isError).toBe(true);
      expect((result.content[0] as { text: string }).text).toContain('Maximum 20 vaults');
    });
    ...
    it('should validate strategy enum', async () => {
      ...
      expect(result.isError).toBe(true);
      expect((result.content[0] as { text: string }).text).toContain('Strategy must be one of');
    });

    it('should validate rebalance threshold range', async () => {
      const invalidInput = {
        ...baseInput,
      ...
      expect(result.isError).toBe(true);
      expect((result.content[0] as { text: string }).text).toContain('cannot exceed 50%');
    });

    it('should validate position values are positive', async () => {
      const invalidInput = {
        ...baseInput,
        currentPositions: [
          { vaultAddress: '0x1234567890123456789012345678901234567890', valueUsd: -1000 },
        ],
      };
